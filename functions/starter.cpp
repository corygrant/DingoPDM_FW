...
MsgCmdResult Starter::ProcessSettingsMsg(PdmConfig* conf, CANRxFrame *rx, CANTxFrame *tx)
{
        ...
        (rx->DLC == 1))
    {
        if (rx->DLC == 5)
        {
            ...
            conf->stStarter.nInput = Dbc::DecodeInt(rx->data8, 16, 16);
            for(int i = 0; i < PDM_NUM_OUTPUTS && i < 8; i++) {
                conf->stStarter.bDisableOut[i] = Dbc::DecodeInt(rx->data8, 32 + i, 1);
            }
        }
        ...
        Dbc::EncodeInt(tx->data8, conf->stStarter.nInput, 16, 16);
        for(int i = 0; i < PDM_NUM_OUTPUTS && i < 8; i++) {
            Dbc::EncodeInt(tx->data8, conf->stStarter.bDisableOut[i], 32 + i, 1);
        }
